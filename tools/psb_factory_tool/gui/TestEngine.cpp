...
int16_t TestEngine::voltsToRaw(double v) const { return psb::reg::voltageFromV(v); }
double  TestEngine::rawToVolts(int16_t raw)   const { return psb::reg::voltageToV(raw); }

TestEngine::TestEngine(QObject* parent) : QObject(parent) {}

void TestEngine::setClient(PsbModbusClient* client, QMutex* mutex) {
    m_client = client;
    m_mutex  = mutex;
}
