...
| Function | Purpose |
|----------|---------|
| `vc_init` | Construct singleton from DTS, start runtime thread, auto-load settings |
| `vc_ctx_start` | Start provider bus (hardware drivers begin sampling) |
| `vc_dispatch` | Submit any command (mode, output, fault, field, calibration, param, measurement) |
| `vc_query` | Read any snapshot or config |
| 14 `vc_cmd_*` builders | Zero-cost command construction |
| 5 `vc_q_*` builders | Zero-cost query construction |

**4 functions + 19 inline builders.** No create, no destroy, no getter.
